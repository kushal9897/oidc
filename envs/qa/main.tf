...
module "ec2_instance" {
  ...
  user_data = <<-EOF
    #!/bin/bash
    echo "QA Environment Instance" > /tmp/environment.txt
...
resource "aws_s3_bucket_server_side_encryption_configuration" "qa_bucket_encryption" {
  bucket = aws_s3_bucket.qa_bucket.id
...
resource "aws_s3_bucket_public_access_block" "qa_bucket_pab" {
  bucket = aws_s3_bucket.qa_bucket.id
...
output "s3_bucket_arn" {
  value = aws_s3_bucket.qa_bucket.arn
}